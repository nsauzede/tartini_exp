...
void HBlockWidget::paintEvent( QPaintEvent * )
{
    ...
    if(l_active_channel)
    {
        ...
        if(l_the_data)
        {
            ...
            QColor l_fill_color = colorBetween(l_color, l_active_channel->get_color(), l_data.getVolumeValue(*gdata));
            QColor l_outline_color = colorBetween(l_color, Qt::black, l_data.getVolumeValue(*gdata));
